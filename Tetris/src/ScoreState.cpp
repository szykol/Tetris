...
ScoreState::ScoreState(int score)
	: m_textBox("Your score is: " + std::to_string(score))
{
	...
	restartButton->setOnClickCallback([] {
		sen::StateManager::pushState<MainGame>();
	});
...
void ScoreState::update(float deltaTime, sf::RenderWindow& window)
{
